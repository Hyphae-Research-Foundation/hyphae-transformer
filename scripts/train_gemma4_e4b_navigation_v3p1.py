#!/usr/bin/env python3
"""Train the preregistered ReZero neuropilot v3.1 over depth-3 calibrated trajectories."""
...
from celiums_rezero.governed.navigation import derive_navigation_dataset_v2
from celiums_rezero.governed.navigation_experiment import (
    HYPHAE_210_SCORE_SCALE,
    run_navigation_experiment_v2,
)
...
def main() -> int:
    ...
    parser.add_argument("--calibration", default=HYPHAE_210_SCORE_SCALE, type=float)
    arguments = parser.parse_args()
    if arguments.calibration != HYPHAE_210_SCORE_SCALE:
        raise SystemExit("navigation v3 calibration is pinned to the Hyphae 2.1.0 scale")
    ...
    if preregistration.get("schema") != (
        ...
        raise SystemExit("navigation v3.1 preregistration schema differs")
    ...
    trajectories = derive_navigation_dataset_v2(
        dataset,
        SufficiencyPolicy(**manifest_values["policy"]),
        provenance=preregistration["dataset"]["provenance"],
        score_scale=arguments.calibration,
        include_continuation=True,
    )
    report = run_navigation_experiment_v2(
        backbone=backbone,
        ...
        calibration_scale=arguments.calibration,
        schema_version="v3.1",
        abstention_ranked=True,
    )
